...
// Based on revision 4931 of https://code.google.com/p/webrtc/source/browse/trunk/samples/js/apprtc/js/main.js
var localVideo;
var miniVideo;
var remoteVideo;
var localStream;
...
function maybeRequestTurn() {
  ...
  if (currentDomain.search('localhost') === -1 &&
      currentDomain.search('127.0.0.1') === -1 &&
  ...
  console.log("Trying turn because domain is ", currentDomain);

  // No TURN server. Get one from computeengineondemand.appspot.com.
  // TODO: we can never do this, can we?
  turnDone = true;
...
function setRemote(message) {
  message.sdp = message.body;
  // Set Opus in Stereo, if stereo enabled.
  if (stereo)
    message.sdp = addStereo(message.sdp);
  message.sdp = maybePreferAudioSendCodec(message.sdp);
  pc.setRemoteDescription(new RTCSessionDescription(message),
       onSetSessionDescriptionSuccess, onSetRemoteSessionDescriptionError);
}
...
function onSetRemoteSessionDescriptionError(error) {
  ...
  console.log('Looking up local session description.', pc);

  // XXX: send our codecs?
      sendMessage({type: 'error',
 		   // XXX: failed-application vs incompatible-parameters,
		   // XEP-0166 vs XEP-0167
		   body: 'failed-application\n' + error.message});

  // So we don't support any of their codecs.  We're supposed to tell
  // them what we _do_ support.
  var constraints = mergeConstraints(offerConstraints, sdpConstraints);
  try {
  pc.createOffer(
    function(sessionDescription) {
      console.log('Got local session description');
      sendMessage({type: 'error',
 		   // XXX: failed-application vs incompatible-parameters,
		   // XEP-0166 vs XEP-0167
		   body: 'failed-application\n' + error.message + '\n'
    ...
    function(secondError) {
      console.log('Failed to get supported codecs while reporting error: ',
		  secondError);
      sendMessage({type: 'error',
 		   // XXX: failed-application vs incompatible-parameters,
		   // XEP-0166 vs XEP-0167
    ...
    console.log('Error while reporting error', e);
  }

  // TODO: this is a fatal error, we should clean up.
}
...
function preferAudioCodec(sdp, codec) {
  ...
  for (var i = 0; i < sdpcLines.length; i++) {
